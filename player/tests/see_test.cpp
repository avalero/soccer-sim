...
TEST(parseSeeMessage, wrongSeeMessage)
{
  string s = "hear 0";
  Player player;
  try
  ...
  {
    EXPECT_STREQ(e.what(), "Invalid see message");
  }
}

TEST(parseSeeMessage, wrongBallMessage)
{
  string s = "(p) 0";
  SeeData see;
  try
  {
    see = Parser::See::parseBallData(s, see);
  }
  catch (const std::exception &e)
  {
    EXPECT_STREQ(e.what(), "Invalid ball message");
  }
}
